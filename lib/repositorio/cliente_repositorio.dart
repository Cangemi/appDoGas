...
class ClienteRepositorio {
  ...
  Future<List> recuperarCliente() async {
    http.Response response =
        await http.get(Uri.parse("$urlBase/admin/cliente/"));
  ...
  Future<Cliente> recuperarClienteLogin(email, senha) async {
    http.Response response =
        await http.get(Uri.parse("$urlBase/cliente/${email}/${senha}"));
    var dadosJson = json.decode(response.body);
  ...
  Future<Cliente> recuperarClienteId(id) async {
    http.Response response =
        await http.get(Uri.parse("$urlBase/cliente/${id}"));
    var dadosJson = json.decode(response.body);
    Cliente c = Cliente(null, '', '', '', '', '');
    if (response.statusCode == 200) {
      c.email = dadosJson["email"];
  ...
  Post(nome, email, senha, tel1, tel2) async {
    ...
    var corpo = json.encode(c.toJson());
    http.Response response = await http.post(Uri.parse("$urlBase/cliente/"),
  ...
  Put(id, nome, email, senha, tel1, tel2) async {
    ...
    var corpo = json.encode(c.toJson());
    http.Response response = await http.put(Uri.parse("$urlBase/cliente/"),
        headers: {"Content-type": "application/json; charset=UTF-8"},
        body: corpo);
    print("resposta: ${response.statusCode}");
    print("resposta: ${response.body}");
