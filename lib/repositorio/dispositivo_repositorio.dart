...
class DispositivoRepositorio {
  ...
  Future<Dispositivo> recuperarDispositivoMAC(String MAC) async {
    http.Response response =
        await http.get(Uri.parse("$urlBase/device/address/$MAC"));
    var dadosJson = json.decode(response.body);
    Dispositivo d =
        Dispositivo(id: null, idCliente: null, nome: '', mac: '', lock: 0);
    if (response.statusCode == 200) {
      d.id = dadosJson["id"];
  ...
  Post(idCli, nome, mac) async {
    Dispositivo d =
        Dispositivo(id: null, idCliente: idCli, nome: nome, mac: mac, lock: 0);
  ...
  Put(id, idCli, nome, mac, lock) async {
    Dispositivo d =
        Dispositivo(id: id, idCliente: idCli, nome: nome, mac: mac, lock: lock);
